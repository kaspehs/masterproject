...
def compute_theta_series(
    ...
    phi_vy = np.asarray(phi_vy, dtype=float)
    sig_dy_loc = np.asarray(sig_dy_loc, dtype=float)
    sig_ddy_loc = np.asarray(sig_ddy_loc, dtype=float)
    speed_mag = np.sqrt(float(flow_speed_m_s) ** 2 + dy**2)
    projection = float(flow_speed_m_s) / np.maximum(speed_mag, np.finfo(float).eps)
    dy_r = dy * projection
    ddy_r = ddy * projection
    cos_phi_dy = dy_r / (sig_dy_loc + np.spacing(1.0))
    sin_phi_dy = -ddy_r / (sig_ddy_loc + np.spacing(1.0))
    phi_dy = np.angle(cos_phi_dy + 1j * sin_phi_dy)
    return wrap_phase(phi_dy - phi_vy, mode=mode)
...
def replay_hidden_state_with_cfd_motion(
    ...
    dy = np.asarray(dy, dtype=float)
    ddy = np.asarray(ddy, dtype=float)
    if not (time.size == y.size == dy.size == ddy.size):
        raise ValueError("time, y, dy, and ddy must have the same length.")
    if time.size < 2:
        raise ValueError("Need at least two samples for hidden-state replay.")
...
def compute_force_spread_history(
    ...
    force_dim = _extract_case_channel(case_payload, "force_dim", "y_force_dim")
