...
SURROGATE_VALIDATION_FILENAME = "surrogate_validation_points.npz"
...
from training.methods.standalone.trainer import (
    LatentRNNForceModel,
...
def _load_surrogate_validation_rows(path: Path) -> list[dict[str, Any]]:
    path = Path(path)
    if not path.exists():
...
def _td_hnn_traj_to_tensors(
    ...
    mass_value = float(np.asarray(traj["dry_mass_kg"]).reshape(()))
...
def _run_hnn_validation(
    *,
    ...
    hnn_cfg = dict(cfg.correction or {})
    loss_cfg = cfg.loss
    ...
    raw_model_cfg = cfg_raw.get("model", {}) if isinstance(cfg_raw, dict) else {}
    ...
    if do_losses:
        ...
        if mean_reg_norm not in {"l1", "l2"}:
            raise ValueError("constraints.mean_reg_norm must be one of: l1, l2.")
...
def _run_hnn_td_correction_validation(
    *,
    ...
    loss_cfg = cfg.loss
    constraints_cfg = cfg.constraints
    vivana_td_cfg = asdict(cfg.vivana_td)
    ...
    correction_mode = str(ckpt.get("correction_mode", resolve_td_correction_mode(asdict(cfg.model)))).strip().lower()
    mode_flags = td_correction_mode_flags(correction_mode)
    mean_active = bool(mode_flags["mean_active"])
    predict_sigma = False
    fhat_active = bool(mode_flags["fhat_active"])
    ...
    input_config_source = asdict(cfg.model)
    ...
    td_force_input_source = str(ckpt.get("td_force_input_source", mean_input_cfg.get("td_force_input_source", "none")))
    use_td_force_input = any(bool(config.get("use_td_force_input", False)) for config in td_input_configs.values())
    use_td_fhat_input = any(bool(config.get("use_td_fhat_input", False)) for config in td_input_configs.values())
    use_acceleration_input = any(bool(config.get("use_acceleration_input", False)) for config in td_input_configs.values())
    phase_input_source = str(mean_input_cfg.get("phase_input_source", "none"))
    use_phi_input = any(bool(config.get("use_phi_input", False)) for config in td_input_configs.values())
    use_sigma_inputs = any(bool(config.get("use_sigma_inputs", False)) for config in td_input_configs.values())
    ...
    fhat_bound_multiplier = float(ckpt.get("fhat_bound_multiplier", getattr(constraints_cfg, "fhat_bound_multiplier", 1.5)))
    ...
    if state_loss_mode != "mse":
        raise ValueError("hnn.state_loss_mode now only supports mse.")
    ...
    mean_reg_norm = str(getattr(constraints_cfg, "mean_reg_norm", "l1")).strip().lower()
    sigma_reg_norm = "l2"
    surrogate_tag = SURROGATE_VALIDATION_TAG
    combined_tag = COMBINED_VALIDATION_TAG
    surrogate_path = train_series_root / SURROGATE_VALIDATION_FILENAME
    surrogate_rows = _load_surrogate_validation_rows(surrogate_path)
    ...
    print(
        ...
        f"from {surrogate_path} (rollout_steps={surrogate_step_counts})"
    )
    ...
    def _run_split(split_tag: str, split_trajs_np: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if do_losses and val_loader is not None:
            val_sums = {
                ...
                for name in ["loss_total", "loss_state", "loss_reg_mean", "loss_reg_sigma", "loss_reg_fhat"]
            }
            val_count = 0
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    z_i, t_i, z_next, t_next, ur_i, _force_true_next, td_context_i, mass_i, damping_i, stiffness_i = batch
        ...
        if do_rollout:
            sampled_metric_indices = list(range(len(split_trajs_np)))
            sampled_names = [str(split_trajs_np[idx].get("name", f"traj_{idx}")) for idx in sampled_metric_indices]
            print(
                f"[async-val][phnn][{split_tag}] epoch {epoch + 1}: metric trajectories={sampled_names} "
            ...
            split_ur_values = [
                ...
                for traj in split_trajs_np
                if np.asarray(traj["ur"]).reshape(-1).size > 0
            ]
            rollout_idx = _rollout_index(
                len(split_trajs_np),
                ur_values=split_ur_values,
            )
    ...
    if surrogate_rows and do_rollout:
...
def _run_latent_rnn_validation(
    *,
    ...
    constraints_cfg = cfg.constraints
    monitoring_cfg = cfg.monitoring
    lrnn_cfg = dict(cfg.standalone or {})
    non_blocking = device.type == "cuda"
    ...
    model = LatentRNNForceModel(
        ...
        coefficient_output_bound=(
            None
            if getattr(constraints_cfg, "coefficient_output_bound", None) is None
            else float(constraints_cfg.coefficient_output_bound)
        ),
        input_scaling_mode=input_scaling_mode,
        ur_scale=ur_scale,
        latent_time_scale=latent_time_scale,
        corr_init_mode=str(getattr(cfg.training, "corr_init_mode", lrnn_cfg.get("corr_init_mode", "zero"))),
    ).to(device)
    _load_state(model, ckpt["model_state"])
    model.eval()
    ...
    combined_tag = COMBINED_VALIDATION_TAG
    surrogate_path = train_series_root / SURROGATE_VALIDATION_FILENAME
    surrogate_rows = _load_latent_surrogate_validation_rows(surrogate_path)
    ...
    if surrogate_rows and not surrogate_encoder_references:
    ...
    def _run_split(split_tag: str, split_trajs: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if do_rollout and split_trajs:
            sampled_indices = list(range(len(split_trajs)))
            sampled_names = [str(split_trajs[idx].get("name", f"traj_{idx}")) for idx in sampled_indices if idx < len(split_trajs)]
            print(
                f"[async-val][latent_rnn][{split_tag}] epoch {epoch}: metric trajectories={sampled_names} "
                f"(mass=dry_mass_kg)"
            )
            metrics_sum: dict[str, float] = {}
            metrics_count: dict[str, int] = {}
    ...
    *,
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for u, res_v, reg_v, mean_v in zip(ur_vals, per_res_vals, per_reg_vals, per_reg_mean_vals):
    ...
    try:
        ...
        if method_key == "correction":
            if not bool(ckpt.get("td_correction", False)):
                raise ValueError("Correction async validation requires a TD-correction checkpoint.")
