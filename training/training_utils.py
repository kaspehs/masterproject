...
from training.architectures import FourierFeatures, ODEPirateNet
...
# Backward-compatible alias for older validation summaries.
...
class DataConfig:
    ...
    train_series_dir: str = "vivana_cfd_data_pipeline/generated/td_burnin_trimmed"
    ...
    *,
    ...
    if str(cfg["mode"]) == "fixed_n_memory":
        return torch.full_like(
            flow_tensor,
            fill_value=max(1.0, float(round(float(params["n_memory"])))),
            dtype=dtype,
            device=device,
        )
    ...
    return torch.clamp(torch.round(tau_tensor / dt_tensor), min=1.0)
...
def td_baseline_step_torch(
    *,
    ...
    fcv = 0.5 * float(rho) * float(diameter) * float(params["Cv"]) * speed_mag * flow_speed * torch.cos(phi_vy_force)
