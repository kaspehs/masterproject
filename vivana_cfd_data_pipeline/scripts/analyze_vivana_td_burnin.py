...
OUTPUT_DIR = DATA_ROOT / "outputs" / "td_burnin_analysis"
OVERWRITE = True
...
COMBINED_THETA_TITLE_FONTSIZE = 9
COMBINED_THETA_LABEL_FONTSIZE = 9
COMBINED_THETA_TICK_FONTSIZE = 8
...
def _time_spread_analysis(
    *,
    ...
    force_traj_rows: list[dict[str, float | str]] = []
    if force_total_stack.ndim == 2 and force_total_stack.shape[1] == time_from_start_s.size:
        for theta_idx, theta0 in enumerate(theta0_values):
            for time_idx, time_value in enumerate(time_from_start_s):
                force_traj_rows.append(
                    {
                        "time_from_start_s": float(time_value),
                        "theta0": float(theta0),
                        "force_total": float(force_total_stack[theta_idx, time_idx]),
                        "case_name": str(case["case_name"]),
                    }
                )
    force_traj_df = pd.DataFrame(force_traj_rows)
    return {
        "summary": summary,
        "theta_trajectories": theta_traj_df,
        "force_trajectories": force_traj_df,
...
def _plot_theta_trajectories_all_cases(
    *,
    ...
    fig_height = min(7.4, max(3.1, 0.95 * n_cases + 0.45))
    rc_params = {
        "font.family": "serif",
    ...
    with plt.rc_context(rc_params):
        fig, axes = plt.subplots(n_cases, 1, figsize=(5.85, fig_height), sharex=True)
        if n_cases == 1:
            axes = [axes]

        for ax, (_case_name, time_from_start_s, theta_stack, theta0_values, summary) in zip(axes, selected_bundles):
            burnin_time = _estimate_burnin_time_from_summary(summary)
            ...
            for idx, _theta0 in enumerate(np.asarray(theta0_values, dtype=float).reshape(-1)):
                ax.plot(
                    time_arr[mask],
                    theta_arr[idx][mask],
                    color=INITIALIZATION_COLORS[idx % len(INITIALIZATION_COLORS)],
                    linewidth=0.85,
                    alpha=0.68,
                )
            if np.isfinite(burnin_time):
                ax.axvline(float(burnin_time), color="0.25", linestyle="--", linewidth=0.9, alpha=0.85)
            title = rf"$U_r={stored_ur:.2f}$" if np.isfinite(stored_ur) else r"$U_r=\mathrm{n/a}$"
            ax.set_title(title, loc="left", fontsize=COMBINED_THETA_TITLE_FONTSIZE, pad=2.0)
            ...
            ax.tick_params(axis="both", labelsize=COMBINED_THETA_TICK_FONTSIZE, length=3.0, width=0.8)
        axes[-1].set_xlabel(r"$t$ [s]", fontsize=COMBINED_THETA_LABEL_FONTSIZE)
        fig.supylabel(r"$\theta_y$ [rad]", fontsize=COMBINED_THETA_LABEL_FONTSIZE, x=0.032)
        fig.tight_layout(rect=(0.03, 0.0, 1.0, 1.0), h_pad=0.25)
        fig.savefig(out_path, dpi=int(COMBINED_THETA_FIG_DPI), bbox_inches="tight", pad_inches=0.02)
        plt.close(fig)


def _plot_force_trajectories_all_cases(
    *,
    case_force_trajectories: list[tuple[str, np.ndarray, np.ndarray, np.ndarray, pd.DataFrame]],
    out_path: Path,
) -> None:
    selected_bundles = _select_theta_trajectory_bundles(case_force_trajectories)
    if not selected_bundles:
        raise ValueError("Need at least one case force trajectory bundle.")
    n_cases = len(selected_bundles)
    fig_height = min(7.4, max(3.1, 0.95 * n_cases + 0.45))
    rc_params = {
        "font.family": "serif",
    ...
    with plt.rc_context(rc_params):
        ...
        for ax, (_case_name, time_from_start_s, force_total_stack, theta0_values, summary) in zip(axes, selected_bundles):
            burnin_time = _estimate_burnin_time_from_summary(summary)
            stored_ur = _stored_ur_display_value(summary)
            time_arr = np.asarray(time_from_start_s, dtype=float)
            force_arr = np.asarray(force_total_stack, dtype=float)
            if COMBINED_THETA_MAX_SECONDS is None:
                mask = np.ones_like(time_arr, dtype=bool)
            else:
                mask = time_arr <= float(COMBINED_THETA_MAX_SECONDS)
            ...
            ax.tick_params(axis="both", labelsize=COMBINED_THETA_TICK_FONTSIZE, length=3.0, width=0.8)
        axes[-1].set_xlabel(r"$t$ [s]", fontsize=COMBINED_THETA_LABEL_FONTSIZE)
        fig.supylabel(r"$F_{\mathrm{hyd}}$ [N/m]", fontsize=COMBINED_THETA_LABEL_FONTSIZE, x=0.032)
        fig.tight_layout(rect=(0.03, 0.0, 1.0, 1.0), h_pad=0.25)
        fig.savefig(out_path, dpi=int(COMBINED_THETA_FIG_DPI), bbox_inches="tight", pad_inches=0.02)
        plt.close(fig)
...
def _write_combined_time_spread_outputs(
    *,
    ...
    force_trajectory_bundles: list[tuple[str, np.ndarray, np.ndarray, np.ndarray, pd.DataFrame]],
    out_dir: Path,
) -> None:
    ...
    _plot_theta_trajectories_all_cases(
        ...
        out_path=out_dir / "theta_vs_time_all_cases.pdf",
    )
    _plot_force_trajectories_all_cases(
        case_force_trajectories=force_trajectory_bundles,
        out_path=out_dir / "force_vs_time_all_cases.pdf",
    )
...
def main() -> None:
    ...
    combined_force_trajectories_by_param = {_paramset_id(params): [] for params in paramsets}
    ...
    for npz_path in _progress(input_npzs, total=len(input_npzs), desc="Cases"):
        ...
        for params in _progress(paramsets, total=len(paramsets), desc=f"{case['case_name']} paramsets"):
            ...
            force_traj_df = pd.DataFrame(analysis["force_trajectories"]).sort_values(["theta0", "time_from_start_s"]).reset_index(drop=True)
            ...
            force_traj_df.to_csv(param_dir / "force_vs_time.csv", index=False)
            ...
            combined_force_trajectories_by_param[_paramset_id(params)].append(
                (
                    str(case["case_name"]),
                    np.asarray(analysis["time_from_start_s"], dtype=float),
                    np.asarray(analysis["force_total_stack"], dtype=float),
                    np.asarray(analysis["theta0_values"], dtype=float),
                    phase_spread_summary.copy(),
                )
            )
        ...
        for params in paramsets:
            ...
            if summaries:
                _write_combined_time_spread_outputs(
                    ...
                    force_trajectory_bundles=combined_force_trajectories_by_param[param_id],
