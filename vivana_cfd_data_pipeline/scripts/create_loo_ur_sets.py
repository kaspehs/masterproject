...
``vivana_cfd_data_pipeline/generated/td_burnin_trimmed4`` with one label U_r omitted. A
...
try:
    ...
    from vivana_cfd_data_pipeline.scripts.generate_surrogate_validation_points import (
        EXCLUDE_UR_ATOL,
        INCLUDE_ANCHOR_POINTS,
        INTERPOLATION_KIND,
        POINTS_PER_INTERVAL,
...
SOURCE_ROOT = Path("vivana_cfd_data_pipeline/generated/td_burnin_trimmed4")
OUTPUT_ROOT = Path("vivana_cfd_data_pipeline/generated/loo_ur_td_burnin_trimmed4_2")
...
def _generate_surrogate_files(loo_dir: Path, held_out_ur: float) -> dict[str, np.ndarray]:
    ...
    save_diagnostic_plot(
        points,
        anchors=anchors,
        output_path=loo_dir / "surrogate_validation_points_diagnostic.pdf",
    )
    return points
...
def _validate_loo_folder(loo_dir: Path, held_out_ur: float) -> LooSummary:
    ...
    required = [
        ...
        loo_dir / "surrogate_validation_points_diagnostic.pdf",
    ]
