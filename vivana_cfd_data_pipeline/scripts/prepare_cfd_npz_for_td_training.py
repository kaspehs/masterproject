...
try:
    ...
    from vivana_cfd_data_pipeline.scripts import analyze_vivana_td_burnin as burnin_config
...
OUTPUT_DIR = DATA_ROOT / "generated" / "td_multi_theta0_baseline"
...
DIAGNOSTIC_PLOT_DIR = DATA_ROOT / "outputs" / "td_preprocess_diagnostics"
...
def _format_td_params(params: dict[str, float]) -> str:
    ...
    parts = [f"{key}={float(params[key]):.6g}" for key in ordered_keys]
    if "force_phase_convention" in params:
        parts.append(f"force_phase_convention={params['force_phase_convention']}")
    return ", ".join(parts)
...
def _build_case_td_replay_inputs(
    ...
    *,
    ...
    params["force_phase_convention"] = str(FORCE_PHASE_CONVENTION)
...
def _manifest_row_base(npz_path: Path, payload: dict[str, np.ndarray], td_paramset_id: str) -> dict[str, object]:
    return {
        ...
        "td_force_phase_convention": str(FORCE_PHASE_CONVENTION),
...
def _prepare_output_payload(
    ...
    *,
    ...
    trimmed["td_force_phase_convention"] = np.asarray(str(FORCE_PHASE_CONVENTION))
