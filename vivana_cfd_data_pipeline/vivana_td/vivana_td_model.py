...
DEFAULT_PARAMS: dict[str, float | str | bool] = {
    "M": 3.0,
    "C": 0.0e-4,
    "K": 7.4 / 4.0,
    "rho": 1.0,
...
def vforce_cf(
    ...
    sig_dy_r: float,
    sig_ddy_r: float,
    force_phase_convention: str = "current",
) -> tuple[float, float, float, float, float, float, float]:
    speed_mag = float(np.sqrt(float(U) ** 2 + float(dy) ** 2))
    projection = float(U) / (speed_mag + np.spacing(1.0))
    dy_r = float(dy) * projection
    ddy_r = float(ddy) * projection
    n_memory = max(1, int(N))

    sig_dy_next = float(np.sqrt((n_memory - 1.0) / n_memory * float(sig_dy_r) ** 2 + dy_r**2 / n_memory))
    sig_ddy_next = float(np.sqrt((n_memory - 1.0) / n_memory * float(sig_ddy_r) ** 2 + ddy_r**2 / n_memory))

    cos_phi_dy = dy_r / (sig_dy_next + np.spacing(1.0))
    sin_phi_dy = -ddy_r / (sig_ddy_next + np.spacing(1.0))
    phi_dy = float(np.angle(complex(cos_phi_dy, sin_phi_dy)))
    theta = float(np.angle(complex(np.cos(phi_dy - float(phi_vy)), np.sin(phi_dy - float(phi_vy)))))
    if theta <= 0.0:
        fhat = float(fhat0) + (float(fhat0) - float(fhat_min)) * np.sin(theta)
    else:
        fhat = float(fhat0) + (float(fhat_max) - float(fhat0)) * np.sin(theta)

    omega_vy = float(2.0 * np.pi * fhat * speed_mag / float(D))
    phi_vy_next = float(phi_vy) + omega_vy * float(dt)
    phase_mode = str(force_phase_convention).strip().lower()
    if phase_mode in {"next", "advanced", "new"}:
        phi_vy_force = phi_vy_next
    elif phase_mode in {"current", "old", "previous"}:
        phi_vy_force = float(phi_vy)
    else:
        raise ValueError(
            "force_phase_convention must be 'next' or 'current', "
            f"got {force_phase_convention!r}."
        )

    Fdy = float(-0.5 * float(rho) * float(D) * float(Cd) * speed_mag * float(dy))
    Fcv = float(0.5 * float(rho) * float(D) * float(Cv) * speed_mag * float(U) * np.cos(phi_vy_force))
    Fca = float(-0.25 * float(rho) * float(Ca) * np.pi * float(D) ** 2 * float(ddy))
    Fy = float(Fcv + Fdy + Fca)
    return Fy, phi_vy_next, sig_dy_next, sig_ddy_next, Fca, Fcv, Fdy


def _structural_acceleration(y: float, dy: float, force: float, model: dict[str, float | str | bool]) -> float:
    return float((-float(model["C"]) * dy - float(model["K"]) * y + force) / float(model["M"]))


def _vivana_added_mass_coeff(model: dict[str, float | str | bool]) -> float:
    return float(0.25 * float(model["rho"]) * float(model["Ca"]) * np.pi * float(model["D"]) ** 2)


def _step_model_for_added_mass_lhs(model: dict[str, float | str | bool]) -> dict[str, float | str | bool]:
    if not bool(model.get("use_added_mass_lhs", False)):
        return model
    step_model = dict(model)
    step_model["M"] = float(model["M"]) + _vivana_added_mass_coeff(model)
    return step_model
...
def _coupled_diagnostics(
    ...
    added_mass_coeff = _vivana_added_mass_coeff(model)
...
def _step_forward_euler(y: float, dy: float, dt: float, force: float, model: dict[str, float | str | bool]) -> tuple[float, float, float]:
...
def _step_rk4(y: float, dy: float, dt: float, force: float, model: dict[str, float | str | bool]) -> tuple[float, float, float]:
...
def run_simulation(
...
) -> dict[str, np.ndarray | dict[str, float | str | bool]]:
    model: dict[str, float | str | bool] = dict(DEFAULT_PARAMS)
    if params:
        model.update(params)
    ...
    for idx in range(n_samples - 1):
        ...
        (
            ...
            str(model.get("force_phase_convention", "current")),
        )
        q_dy_loc[idx + 1] = sig_dy_loc[idx + 1] ** 2
        q_ddy_loc[idx + 1] = sig_ddy_loc[idx + 1] ** 2
        step_force = float(Fy[idx + 1])
        step_model = model
        if bool(model.get("use_added_mass_lhs", False)):
            step_force = float(Fcv[idx + 1] + Fdy[idx + 1])
            step_model = _step_model_for_added_mass_lhs(model)
        ...
        if bool(model.get("use_added_mass_lhs", False)):
            Fca[idx + 1] = float(-_vivana_added_mass_coeff(model) * ddy[idx + 1])
            Fy[idx + 1] = float(Fcv[idx + 1] + Fdy[idx + 1] + Fca[idx + 1])
    ...
    *,
    ...
    for idx in range(n_samples):
        ...
        (
            ...
            float(sig_dy_loc[idx]),
            float(sig_ddy_loc[idx]),
            str(params.get("force_phase_convention", "current")),
        )
